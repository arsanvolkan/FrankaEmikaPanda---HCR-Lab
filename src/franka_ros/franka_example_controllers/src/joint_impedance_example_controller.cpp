...
void Write_To_File(std::string file_name, std::vector<std::vector<double>> dataToSave) { // modified : added function to save data in file
  std::ofstream myfile;
  ...
  {
    for (int j = 0; j < dataToSave[i].size(); j++)
    {
      if (j == dataToSave[i].size() - 1) {
        myfile << dataToSave[i][j] << " ";
      }
      else {
        myfile << dataToSave[i][j] << "  ";
      }
...
void Write_Line_To_File(std::string file_name, std::vector<double> position, Eigen::Quaterniond orientation, ros::Duration time) { // modified : added function to save data in file
  std::ofstream myfile;
  ...
  myfile << orientation.w() << " " << orientation.x() << " " << orientation.y() << " " << orientation.z() << " " << time << std::endl;


  myfile.close();
}

void Write_velocity_To_File(std::string file_name, std::vector<double> position, Eigen::Quaterniond orientation, double velocity_x, double velocity_y, double velocity_z, ros::Duration time) { // modified : added function to save data in file
  std::ofstream myfile;
  myfile.open(file_name, std::ios_base::app);
  if (!myfile)
  {
...
//  }
 // myfile << orientation.w() << " " << orientation.x() << " " << orientation.y() << " " << orientation.z() << " " << time << std::endl;
  myfile << velocity_x << " " << velocity_y << " " << velocity_z << " " << time << std::endl;
...
void JointImpedanceExampleController::starting(const ros::Time& time) {
  ...
  initial_previous_pose_ = cartesian_pose_handle_->getRobotState().O_T_EE;
  initial_time_ = time;
}
...
void JointImpedanceExampleController::update(const ros::Time& time,
  ...
  for (size_t i = 0; i < 7; ++i) {
    tau_d_calculated[i] = coriolis_factor_ * coriolis[i] +
                         0* k_gains_[i] * (robot_state.q_d[i] - robot_state.q[i]) +
                          0.2*  d_gains_[i] * (0*robot_state.dq_d[i] - dq_filtered_[i]);
  }
  ...
  Eigen::Affine3d transform_previous(Eigen::Matrix4d::Map(initial_previous_pose_.data()));
  Eigen::Vector3d position(transform.translation());
  Eigen::Vector3d position_previous(transform_previous.translation());
  Eigen::Quaterniond orientation(transform.linear());
  double velocity_x = position.x() - position_previous.x();
  double velocity_y = position.y() - position_previous.y();
  double velocity_z = position.z() - position_previous.z();
  initial_previous_pose_ = robot_state.O_T_EE;
  std::vector<double> v1;
  v1.resize(position.size());
  Eigen::Vector3d::Map(&v1[0], position.size()) = position;

  //ROS_INFO("%lf",v1[0]);
  Write_Line_To_File("/home/dhrikarl/catkin_ws/joint_impedance_position_schoolVisit.txt",v1,orientation,time-initial_time_ );
  Write_velocity_To_File("/home/dhrikarl/catkin_ws/joint_impedance_position_schoolVisit_diffPose.txt", v1, orientation, velocity_x, velocity_y, velocity_z, time-initial_time_);
Write_velocity_To_File("/home/dhrikarl/catkin_ws/joint_impedance_position_schoolVisit_velocity.txt", v1, orientation, velocity_x/period.toSec(), velocity_y/period.toSec(), velocity_z/period.toSec(), time-initial_time_);
}
