...
#include <franka_example_controllers/main_controller.h>
...
bool MainController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("MainController: Could not get parameter arm_id");
    return false;
  }
  ...
  try {
    ...
    ROS_ERROR_STREAM("MainController: Exception getting Cartesian handle: " << e.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR("MainController: Could not get state interface from hardware");
    return false;
  }
  ...
  try {
    ...
    for (size_t i = 0; i < q_start.size(); i++) {
      if (std::abs(state_handle.getRobotState().q_d[i] - q_start[i]) > 0.1) {
        ROS_ERROR_STREAM(
            "MainController: Robot is not in the expected starting position for "
    ...
    ROS_ERROR_STREAM("MainController: Exception getting state handle: " << e.what());
    return false;
  }
...
void MainController::update(const ros::Time& /* time */, const ros::Duration& period) {
  ...
  if (move == 1) {
    x = 0.55, y = -0.35, z = 0.14;  // Go to the Object coordinates
    F_x = 1, F_y = 1, F_z = 1;
    Move_Width = 0.08;
    franka_gripper::MoveGoal goal1;
    goal1.width = Move_Width;
    goal1.speed = 0.05;  //  Closing speed. [m/s]
    ac1.sendGoal(goal1);
  }
//  if (move == 2){
//    x = -0.3, y = 0, z = 0.5;
...
//
//  }
//  if (move == 2) {  // go down to pick up the fruit
//    z = 0.06;
//    F_x = 0, F_y = 0, F_z = 1;
//  }
...
//    F_x = 0, F_y = 0, F_z = 1;
//  }
  if (move == 2) {  // go to HOME position after the object had been released
    x = x_home, y = y_home, z = z_home;
    F_x = 1, F_y = -1, F_z = 1;
  }

//  if (move == 6) {  // Move the object to the new coordinates
//    x = 0.50, y = 0.15, z = 0.10;
//    F_x = 1, F_y = -1, F_z = 1;
//  }
//
//  if (move == 7 & schritt == 1) {  // release the object
//    Grasp_Width = 0.08;
//    schritt = 2;
//  }
//
//  if (move == 8) {  // go to HOME position after the object had been released
//    x = x_home, y = y_home, z = z_home;
//    F_x = 1, F_y = -1, F_z = 1;
//  }
//
//  if (move == 9) {
//    x = 0.55, y = 0.00, z = 0.12;  // Go to the Object coordinates
//    F_x = 1, F_y = 1, F_z = 1;
//    Move_Width = 0.08;
//    franka_gripper::MoveGoal goal1;
//    goal1.width = Move_Width;
...
//  }
//
//  if (move == 10) {  // go down to pick up the knife
//    z = 0.05;
//    F_x = 0, F_y = 0, F_z = 1;
//  }
//  if (move == 11 & schritt == 1) {  // grasp the knife
//    schritt = 2;
//    Grasp_Width = 0.038;
//
//    schritt = 2;
//  }
//
//  if (move == 12) {  // go up after grasping the object
//    z = 0.35;
//    F_x = 0, F_y = 0, F_z = 1;
//  }
//  if (move == 13) {  // Move the object to the new coordinates
//    x = 0.60, y = 0.13, z = 0.14;
//    F_x = 1, F_y = -1, F_z = 1;
//  }
//
//  if (move == 14 & schritt == 1) {  // cut motion
//    z = 0.05;
//    F_x = 0, F_y = 0, F_z = 1;
//  }
//  if (move == 15) {  // go to HOME position after the object had been released
//    x = x_home, y = y_home, z = z_home;
//    F_x = 1, F_y = -1, F_z = 1;
//  }

  if (move == 3) {  // Programm termination
    schritt = 9;    //  For repeatitive task please set schritt to (1). or 9 otherwise
    Pcheck = 0;
    move = 1;
    // exit(0);      // un42comment this for program termination
  }
  // ********************************************************************************************
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::MainController,
