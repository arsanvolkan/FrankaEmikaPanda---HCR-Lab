...
//  elapsed_time_ += period;
//  vector<float> joint_position{ 0.0032727483566246107, -0.007488883987739392, -0.003146508670865522, -1.5824404135826011, 0.00016863201193473287, 1.5907779233793136, 0.7865727289892712 };

//  double delta_angle = M_PI / 16 * (1 - std::cos(M_PI / 5.0 * elapsed_time_.toSec())) * 0.2;
//  for (size_t i = 0; i < 7; ++i) {

//    position_joint_handles_[i].setCommand(joint_position[i]);
